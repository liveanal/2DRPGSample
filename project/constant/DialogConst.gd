...
static func FullDialog():
	var dialog:Dialog = load("res://project/node/dialog/general/FullDialog.tscn").instantiate()
	dialog.finished_close.connect(dialog.queue_free)
	return dialog

# nofaceダイアログ
static func NoFaceDialog():
	var dialog:Dialog = load("res://project/node/dialog/general/NoFaceDialog.tscn").instantiate()
	dialog.finished_close.connect(dialog.queue_free)
	return dialog

# infoダイアログ
static func InfoDialog():
	var dialog:Dialog = load("res://project/node/dialog/general/InfoDialog.tscn").instantiate()
	dialog.finished_close.connect(dialog.queue_free)
	return dialog
