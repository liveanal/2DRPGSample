...
static func QuitModal():
	var modal:Modal = load("res://project/node/modal/general/QuitModal.tscn").instantiate()
	modal.finished_close.connect(func():modal.queue_free())
	return modal

# Optionモーダル
static func OptionModal():
	var modal:Modal = load("res://project/node/modal/general/OptionModal.tscn").instantiate()
