...
const balloon_icon_res  := preload("res://project/node/balloon/general/IconBalloon.tscn")
const balloon_image_res := preload("res://project/node/balloon/general/ImageBalloon.tscn")
const balloon_text_res  := preload("res://project/node/balloon/general/TextBalloon.tscn")
...
# ログ出力（
func mlog(msg:String):
	print_debug("[INFO]"+msg)
...
# バルーン表示（アイコン）
func balloon_icon(target, offset:Vector2, anim_name:String,iscale:float=1.0,_scale:float=1.0,speed_scale:float=1.0,auto_close:bool=true,anim_time:=0.65,wait_time:=1.5)->IconBalloon:
	var obj := balloon_icon_res.instantiate()
	obj.anim_name=anim_name
	obj.speed_scale=speed_scale
	obj.pos_offset=target.position+offset
...
# バルーン表示（画像）
func balloon_img(target, offset:Vector2, anim_name:String,_scale:float=1.0,speed_scale:float=1.0,auto_close:bool=true,anim_time:=0.65,wait_time:=1.5)->ImageBalloon:
...
# バルーン表示（テキスト）
func balloon_text(target, offset:Vector2, text:String,font_size:=16,_scale:float=1.0,auto_close:bool=true,anim_time:=0.65,wait_time:=1.5)->TextBalloon:
...
func open_menu(party_member:Array[Character]):
	menu.party_member = party_member
	menu.open()
