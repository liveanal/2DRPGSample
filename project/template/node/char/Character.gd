...
const balloon_icon_res  := preload("res://project/template/node/balloon/general/IconBalloon.tscn")
const balloon_image_res := preload("res://project/template/node/balloon/general//ImageBalloon.tscn")
const balloon_text_res  := preload("res://project/template/node/balloon/general//TextBalloon.tscn")
...
# ショートカット1を登録
func entry_smenu1(i:int, entry:SMenuData):
	data.smenu1[i] = entry
	smenu1.entry_select(i,entry)

# ショートカット2を登録
func entry_smenu2(i:int, entry:SMenuData):
	data.smenu2[i] = entry
	smenu2.entry_select(i,entry)

# ショートカット3を登録
func entry_smenu3(i:int, entry:SMenuData):
	data.smenu3[i] = entry
...
# アイテム使用
func use_spell(spell:SpellData):
	pass
