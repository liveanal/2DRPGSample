...
const _LOAD_ENTRY_RESOURCE_PATHS:PackedStringArray = []
...
static func find_top_node(in_node,test:Callable):
	...
	if parent==null:
		...
		return find_top_node(parent,test)
...
# 非同期ロード準備
static func load_to_thread_entry(res_path:Array[String],use_sub_threads:=false) -> Dictionary:
	var errors := {}
	for path in res_path:
		...
		if error:
			...
			_LOAD_ENTRY_RESOURCE_PATHS.append(path)
	return errors

# 非同期ロード
static func load_to_thread_start(await_signal:Signal, error_func:Callable=func(_e):pass, progress_func:Callable=func(_p):pass):
	var progress:={}
	for path in _LOAD_ENTRY_RESOURCE_PATHS:
		progress[path] = [0.0]
	# ロードスレッド開始
	for path in _LOAD_ENTRY_RESOURCE_PATHS:
		...
		thread.start(func():
			while true:
				var status = ResourceLoader.load_threaded_get_status(path, progress[path])
				if status == ResourceLoader.THREAD_LOAD_LOADED:
					break
				elif status == ResourceLoader.THREAD_LOAD_FAILED:
					error_func.call(path)
					_LOAD_ENTRY_RESOURCE_PATHS.remove_at(_LOAD_ENTRY_RESOURCE_PATHS.find(path))
					break
	...
	thread.start(func():
		while true:
			...
			for p in progress.values():
				result += p[0]/float(progress.size())
			progress_func.call(result*100.0)
			
			# ロード状況確認
			var completed := true
			for p in _LOAD_ENTRY_RESOURCE_PATHS:
...
# 非同期ロード取出し
static func load_to_thread_pop() -> Dictionary:
	var result := {}
	for path in _LOAD_ENTRY_RESOURCE_PATHS:
		result[path] = ResourceLoader.load_threaded_get(path)
	_LOAD_ENTRY_RESOURCE_PATHS.clear()
	return result
