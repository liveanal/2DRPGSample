...
func update():
	...
	if data.is_multiple:
		if 0<data.trade_count:
			count.text = count_select_pattern.format({"count":str(data.count), "trade_count":str(data.trade_count)})
			count.label_settings = label_select
		else:
			count.text = count_default_pattern.format({"count":str(data.count), "trade_count":str(data.trade_count)})
